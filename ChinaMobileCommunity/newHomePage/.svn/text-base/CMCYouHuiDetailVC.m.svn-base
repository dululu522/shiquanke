...
@property (nonatomic, strong) NSString *before_price,*discount,*end_time,*id,*information,*is_all,*is_promise,*life,*notice,*price,*store_id,*title,*type;
...
//    [self.myTableView addSubview:self.myScrollview];
...
//            for (NSString *str in [dataDic objectForKey:@"dis_goods"]) {
...
//        cell.detailLable.text = self.discription;
...
//        cell.textLabel.text = self.end_time;
...
//        if ([self.is_promise isEqualToString:@"1"]) {
...
//    
//    if (indexPath.row == 4) {
//        if (self.dis_goodsArr.count != 0) {
//            for (int i = 0; i < self.dis_goodsArr.count; i++) {
//                UILabel *label = [[UILabel alloc]initWithFrame:CGRectMake(10, 20 * i, 100, 20)];
//                label.text = [self.dis_goodsArr objectAtIndex:i];
