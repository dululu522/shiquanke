...
- (void)didClickPayButton:(UIButton *)sender{
    ...
    [BaseUtil post:url postBody:tmpDic success:^(id responseObj) {
        ...
        if ([[[responseObj objectForKey:@"info"] objectForKey:@"result"] intValue] == 3) {
            [BaseUtil toast:@"需要登录"];
        }
...
{
    if ([self isExpress]) {
        NSLog(@"价格%.2f %.2f",[self.price floatValue],[[CMCUserManager shareManager].order_money floatValue]);
